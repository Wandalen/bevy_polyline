use crate::{
  polyline::{DrawPolyline, PolylinePipeline, PolylinePipelineKey, PolylineUniform, PolylineViewBindGroup, SetPolylineBindGroup},
...
{
  fn fragment_shader(_asset_server : &AssetServer) -> Handle<Shader> { FRAG_SHADER_HANDLE.typed() }

  fn vertex_shader(_asset_server : &AssetServer) -> Handle<Shader> { VERT_SHADER_HANDLE.typed() }
    ...
    render_device.create_bind_group_layout(&BindGroupLayoutDescriptor {
      entries : &[BindGroupLayoutEntry {
        ...
        ty : BindingType::Buffer {
          ...
          min_binding_size : BufferSize::new(PolylineMaterialUniform::std140_size_static() as u64),
        },
        count : None,
      }],
  ...
  #[inline]
  fn bind_group(render_asset : &<Self as RenderAsset>::PreparedAsset) -> &BindGroup { &render_asset.bind_group }
...
impl RenderAsset for PolylineMaterial
{
  ...
  fn extract_asset(&self) -> Self::ExtractedAsset { *self }
...
impl SpecializedPipeline for PolylineMaterialPipeline
{
  ...
  fn specialize(&self, key : Self::Key) -> RenderPipelineDescriptor
  {
    ...
    {
      descriptor.vertex.shader_defs.push("POLYLINE_PERSPECTIVE".to_string());
    }
...
{
  type Param = (SRes<RenderAssets<PolylineMaterial>>, SQuery<Read<Handle<PolylineMaterial>>>);
...
{
  for (view, visible_entities, mut opaque_phase, mut alpha_mask_phase, mut transparent_phase) in views.iter_mut()
  {
    let draw_opaque = opaque_draw_functions.read().get_id::<DrawMaterial>().unwrap();
    let draw_alpha_mask = alpha_mask_draw_functions.read().get_id::<DrawMaterial>().unwrap();
    let draw_transparent = transparent_draw_functions.read().get_id::<DrawMaterial>().unwrap();
    ...
    for visible_entity in &visible_entities.entities
    {
      if let Ok((material_handle, polyline_uniform)) = material_meshes.get(*visible_entity)
      {
        if let Some(material) = render_materials.get(material_handle)
        {
          ...
          let pipeline_id = pipelines.specialize(&mut pipeline_cache, &material_pipeline, polyline_key);
