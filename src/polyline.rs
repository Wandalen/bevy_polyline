...
impl RenderAsset for Polyline
{
  ...
  fn extract_asset(&self) -> Self::ExtractedAsset { self.clone() }
...
pub fn extract_polylines(
  ...
  query : Query<(Entity, &ComputedVisibility, &GlobalTransform, &Handle<Polyline>)>,
)
{
...
impl PolylinePipelineKey
{
  ...
  pub fn msaa_samples(&self) -> u32 { ((self.bits >> Self::MSAA_SHIFT_BITS) & Self::MSAA_MASK_BITS) + 1 }
}
...
)
{
  ...
  )
  {
    for (entity, _view_shadow_bindings, _view_cluster_bindings) in views.iter()
    {
      ...
      commands
        .entity(entity)
        .insert(PolylineViewBindGroup { value : view_bind_group });
    }
  }
}
...
{
  type Param = (SRes<PolylineBindGroup>, SQuery<Read<DynamicUniformIndex<PolylineUniform>>>);
  ...
  ) -> RenderCommandResult
  {
    ...
    pass.set_bind_group(I, &polyline_bind_group.into_inner().value, &[polyline_index.index()]);
